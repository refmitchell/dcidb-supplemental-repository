...
Spherical coordinates are needed within the code to define the geometry but they
must be translated to cartesian to be plotted.

This module was used only for the experimental parallax simulator; it plays no 
role in the rest of the system.
"""
...
#    print("R: {}".format(resultant_vector_as_list))
...
def define(src, dst):
    """
    Define a vector point from src to dst
    :param src: The vector startpoint (Vec3)
    :param dst: The endpoint (Vec3)
    :return: src -> dst
    """
    src_list = src.get_cartesian_as_list()
    dst_list = dst.get_cartesian_as_list()

    x, y, z = [ d - s for (d,s) in zip(dst_list, src_list)]
    newvec = Vec3()
    newvec.set_cartesian(x,y,z)
#    print(newvec.get_spherical_as_list())
#    input()
    return newvec
...
class Vec3:
    ...
    def set_cartesian(self, x, y, z):
        ...
        self.__theta = np.pi/2 - np.arccos(z/self.__r)
